...
import { CRATE_LAYOUT, PALLET } from '../src/game/config.js';
...
test('six object clicks carry three crates; empty floor never earns money', () => {
  ...
  for (let box = 1; box <= 3; box++) {
    ...
    assert.equal(state.warehouse.wage, (box - 1) * 120);
    ...
    assert.equal(state.warehouse.wage, box * 120);
  ...
  assert.equal(unchanged.warehouse.wage, 360, 'duplicate drop has no payment');
});
...
test('robot takes the same source and marks every paid delivery for the belt', () => {
  ...
  assert.equal(state.warehouse.wage, 480);
...
test('future is three distinct ordered visions, not a disabled coming-soon button', () => {
  assert.deepEqual(FUTURE_SCENES.map(s => s.id), ['shop', 'vika', 'battle']);
