...
test('первый Python-чип остаётся физическим действием игрока и не телепортирует в терминал', () => {
  assert.match(html, /id=["']pythonChip["']/);
  const main = readFileSync(new URL('../src/game/main.js', import.meta.url), 'utf8');
  const model = readFileSync(new URL('../src/game/model.js', import.meta.url), 'utf8');
  assert.match(main, /query\.get\('showcase'\) === 'chip'/);
  assert.match(main, /firstPersonScene\(\)/);
  assert.doesNotMatch(main, /state\.arm\.chip === 'installed'\) openMachinePanel\(\)/);
  assert.match(model, /chip: 'held'/);
  assert.match(model, /ВСТАВИТЬ ЧИП В РАЗЪЁМ/);
});
...
test('первый экран обещает игру, а не учебный курс', () => {
  assert.match(html, /Не твой участок\./);
  assert.match(html, /Начальник уже ждёт у линии/);
  assert.match(html, /Войди на склад и выясни/);
  const main = readFileSync(new URL('../src/game/main.js', import.meta.url), 'utf8');
  ...
  const start = html.match(/<section id=["']startPanel["'][\s\S]*?<\/section>/)?.[0] ?? '';
  assert.doesNotMatch(start, /коротк|подробн|урок|обучени/i);
});
