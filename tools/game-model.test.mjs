...
test('автобой обрывается дисконнектом и успевает передать игру складу', () => {
  const afterFlash = advance(createGameState(), 8.4);
  assert.equal(afterFlash.scene, 'warehouse');
  assert.equal(afterFlash.prologue.threats, 13);
  assert.equal(afterFlash.prologue.enemies.filter(({ alive }) => alive).length, 11);
});
...
test('после первой смены чип остаётся в руках, а вставка доступна только у разъёма', () => {
  const chip = createCheckpointState('chip');
  assert.equal(chip.arm.chip, 'held');
  assert.equal(getNearbyAction(chip), null);
  chip.player.x = 1010; chip.player.y = 636;
  assert.deepEqual(getNearbyAction(chip), { type: 'insert-python-chip', label: 'ВСТАВИТЬ ЧИП В РАЗЪЁМ' });
});

test('вставленный руками чип запускает руку только после завершения посадки', () => {
...
test('первая принятая команда будит руку и ставит все оставшиеся ящики в очередь', () => {
  const machine = createGameState({ scene: 'machine' });
...
test('после print wake машина сначала заметно просыпается и только потом берёт ящик', () => {
  let state = applyGameAction(createGameState({ scene: 'machine' }), { type: 'first-command-accepted' });
...
test('шесть автоматических переносов видны по одному, затем рука проживает физическую ошибку', () => {
  // Later print/if lesson fixture, not the newly approved chip-only first episode.
  let state = applyGameAction(createGameState({ scene: 'machine' }), { type: 'first-command-accepted' });
  let elapsed = 0;
  const deliveryTimes = [];
  let delivered = 0;
