...
export const pythonLessons = [
  {
    ...
    tier: 'python',
    title: 'Отклик',
    subtitle: 'Заставь чужой терминал заговорить',
    skill: 'print',
    time: '02:14',
    story: 'Ты внутри, и экран пуст. Восемь лет этой системе отдавали приказы и ни разу не спросили. Скажи ей что-нибудь и посмотри, ответит ли: пока она молчит, ты не знаешь, жива она или просто не слушает.',
...

  {
    ...
    tier: 'python',
    title: 'Тайник',
    subtitle: 'Запомни то, что нельзя записывать',
    skill: 'переменные',
    time: '02:26',
    story: 'Позывной, адрес шлюза, код смены. В голове не удержишь, на бумаге найдут. Система умеет помнить за тебя и не спрашивает зачем.',
...

  {
    ...
    tier: 'python',
    title: 'Счётчик',
    subtitle: 'Сколько прошло и сколько осталось',
    skill: 'числа',
    time: '02:38',
    story: 'Сколько попыток осталось, сколько секунд до сброса, сколько их было до тебя. Считать в уме на чужой машине — верный способ уйти отсюда в наручниках.',
    learned: 'Арифметика, целая часть и остаток от деления, проценты.',
    sprint: {
...

  {
    ...
    tier: 'python',
    title: 'Перехват',
    subtitle: 'Вычисти мусор из перехваченного',
    skill: 'строки и f-строки',
    time: '02:50',
    story: 'Журнал пишется не для тебя, но читать его будешь ты. Собери из обрывков строку, по которой потом отличишь свой след от чужого.',
    learned: 'Собирать текст f-строкой и чистить его методами строк.',
    sprint: {
...

  {
    ...
    tier: 'python',
    title: 'Пропуск',
    subtitle: 'Снаружи всё приходит текстом',
    skill: 'int, str, float, input',
    time: '03:02',
    story: 'Всё, что приходит по сети, приходит текстом. Строка «96» и число 96 выглядят одинаково и ведут себя по-разному. На этом ловятся и новички, и те, кто их ловит.',
    learned: 'Различать типы и преобразовывать их: int, str, float, input.',
    sprint: {
...

  {
    ...
    subtitle: 'Сравнение отвечает только да или нет',
    skill: 'bool, сравнения',
    time: '03:14',
    story: 'В три часа ночи здесь не должно быть никого. Кто-то есть: чужая сессия открыта с обеда, и её просто не закрыли. Свой или чужой — вопрос с двумя ответами, и отвечать код обязан быстрее тебя.',
    learned: 'Сравнивать значения и получать True или False.',
    sprint: {
...

  {
    ...
    tier: 'python',
    title: 'Шлюз',
    subtitle: 'Пустить или поднять тревогу',
    skill: 'if / elif / else',
    time: '03:26',
    story: 'У шлюза три ветки: пропустить, придержать, отбить. Выбирает он, а не ты. Пока ты думаешь, ответ уже ушёл.',
    learned: 'Ветвление: if, elif, else и отступы.',
    sprint: {
...

  {
    ...
    subtitle: 'Два условия сразу — и ловушка между ними',
    skill: 'логические операторы',
    time: '03:38',
    story: 'Пускать, только если токен верный и роль из списка. Одного из двух мало. Через такое «или» вместо «и» проходят все, кто потом рассказывает, как это было легко.',
    learned: 'Соединять условия через and, or и not — и не путать их местами.',
    sprint: {
...

  {
    ...
    tier: 'python',
    title: 'Список целей',
    subtitle: 'Много адресов под одним именем',
    skill: 'list, индексы, срезы',
    time: '03:50',
    story: 'Список адресов, список смен, список тех, кто входил сегодня. Пока это строчки в файле, всё просто. Ниже по списку начинаются фамилии.',
    learned: 'Список: индексы с нуля, срезы и добавление элементов.',
    sprint: {
...

  {
    ...
    tier: 'python',
    title: 'Обход',
    subtitle: 'Пройти все цели по очереди',
    skill: 'for, range',
    time: '04:02',
    story: 'Четыреста записей в журнале. Глазами найдёшь то, что лежит сверху. Тот, кто прятал, сверху не клал.',
    learned: 'Цикл for, range и накопление результата.',
    sprint: {
...

  {
    id: 'py-while',
    tier: 'python',
    title: 'Пока не откроется',
    subtitle: 'Повторять, пока есть смысл',
    skill: 'while, break',
    time: '04:12',
    story: 'Сколько раз пробовать, не скажет никто. Пробуют, пока не откроется, и вовремя останавливаются: после пятой попытки система запоминает уже не пароль, а тебя.',
    learned: 'Цикл while, break и удвоение паузы между попытками.',
    sprint: {
...

  {
    ...
    tier: 'python',
    title: 'Досье',
    subtitle: 'Одна запись, много полей',
    skill: 'dict',
    time: '04:20',
    story: 'В системе лежит досье: кто, когда, откуда. По номеру такое не достанешь — только по имени. Это имя ты видел час назад на пропуске в холле.',
    learned: 'Словарь: пары «ключ — значение» и безопасное чтение через get.',
    sprint: {
...

  {
    ...
    tier: 'python',
    title: 'Отмычка',
    subtitle: 'Собери инструмент, которым можно пользоваться дважды',
    skill: 'def, return',
    time: '04:27',
    story: 'Один и тот же приём ты повторил четыре раза. На пятый ошибёшься. Отмычку делают не от лени, а чтобы рука не дрогнула.',
    learned: 'Функция: параметры, return и значения по умолчанию.',
    sprint: {
...

  {
    ...
    tier: 'python',
    title: 'Срыв',
    subtitle: 'Не падать, когда что-то пошло не так',
    skill: 'обработка ошибок',
    time: '04:33',
    story: 'На середине система отдаёт мусор вместо ответа. Программа, которая падает сейчас, оставляет половину следов и половину дыры. Утром сюда придёт человек и не поймёт, почему ничего не работает.',
    learned: 'Перехватывать ожидаемые ошибки и возбуждать свои через raise.',
    sprint: {
...

  {
    ...
    subtitle: 'Собери всё в один рабочий инструмент',
    skill: 'итоговая работа',
    time: '04:37',
    story: 'Ты добрался до панели, с которой видно всё. И увидел: это не абстрактная сеть, а чья-то смена, чей-то отчёт, чья-то зарплата. Отдельно ты умеешь всё. Собери разом и реши, что с этим делать.',
    learned: 'Собирать программу из функций, коллекций и проверок, разделяя обязанности.',
    sprint: {
...

  {
    ...
    time: '04:41',
    story: 'Ты закрыл за собой дверь и оставил записку. Теперь то же самое у себя: напиши защиту, найди дыру в чужой — и докажи, что у тебя её нет. Пройдёшь свою — значит соврал себе.',
    preamble: SERVER,
